...
OpenRelTable::~OpenRelTable() {
    ...
    RelCatEntry relcat;
    Attribute record[RELCAT_NO_ATTRS];RecId recId;
    if (RelCacheTable::relCache[ATTRCAT_RELID]->dirty) {

        /* Get the Relation Catalog entry from RelCacheTable::relCache
        Then convert it to a record using RelCacheTable::relCatEntryToRecord(). */
        
        RelCacheTable::getRelCatEntry(ATTRCAT_RELID,&relcat);
        
        RelCacheTable::relCatEntryToRecord(&relcat,record);
    ...
    for (int i=ATTRCAT_RELID;i>=RELCAT_RELID;i--){
        AttrCacheEntry * curr=AttrCacheTable::attrCache[i], *next=nullptr;
        int offset=0;
        while(curr!=nullptr){
            next=curr->next;
            if(curr->dirty){
                AttrCatEntry atrcat;
                AttrCacheTable::getAttrCatEntry(i,offset,&atrcat);
                Attribute record1[RELCAT_NO_ATTRS];
                AttrCacheTable::attrCatEntryToRecord(&atrcat,record1);

                recId=curr->recId;
                RecBuffer attrCatBlock(recId.block);

                attrCatBlock.setRecord(record1,recId.slot);
...
int OpenRelTable::getRelId(char relName[ATTR_SIZE]) 
{
    ...
    for (int relId = 0; relId < MAX_OPEN; relId++) {
        ...
        if (!strcmp(tableMetaInfo[relId].relName, relName) && tableMetaInfo[relId].free == false){
            return relId;
        }
    }    
