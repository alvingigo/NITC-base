...
RecId BlockAccess::print(int relId)
{
...
int BlockAccess::renameRelation(char oldName[ATTR_SIZE], char newName[ATTR_SIZE]){
    ...
    RecId x=BlockAccess::linearSearch(RELCAT_RELID,RELCAT_ATTR_RELNAME,newRelationName,EQ);
    ...
    x=BlockAccess::linearSearch(RELCAT_RELID,RELCAT_ATTR_RELNAME,oldRelationName,EQ);
    ...
    */
    ...
    RecBuffer Recordbuf(x.block); 
    Attribute Recor[RELCAT_NO_ATTRS];
    Recordbuf.getRecord(Recor,x.slot); 
    Recor[RELCAT_REL_NAME_INDEX]=newRelationName;
   // printf("%s newrelname",Recor[RELCAT_REL_NAME_INDEX].sVal);
    
    Recordbuf.setRecord(Recor,x.slot);
   ...
   RelCacheTable::resetSearchIndex(ATTRCAT_RELID);
    ...
    RecId searchind;
    Attribute atrecord[ATTRCAT_NO_ATTRS];

    /* reset the searchIndex of the attribute catalog using
       RelCacheTable::resetSearchIndex() */
    for(int i =0;i<Recor[RELCAT_NO_ATTRIBUTES_INDEX].nVal;i++){
        searchind=BlockAccess::linearSearch(ATTRCAT_RELID,ATTRCAT_ATTR_RELNAME,oldRelationName,EQ);
        
        
      
         RecBuffer AttrBuf(searchind.block);
         AttrBuf.getRecord(atrecord,searchind.slot);
         atrecord[ATTRCAT_REL_NAME_INDEX]=newRelationName;
         AttrBuf.setRecord(atrecord,searchind.slot);
...
int BlockAccess::renameAttribute(char relName[ATTR_SIZE], char oldName[ATTR_SIZE], char newName[ATTR_SIZE]) {
    ...
    RecId relsearch=BlockAccess::linearSearch(RELCAT_RELID,RELCAT_ATTR_RELNAME,relNameAttr,EQ);
    ...
    while (true) {
        ...
        attrToRenameRecId=BlockAccess::linearSearch(ATTRCAT_RELID,ATTRCAT_ATTR_RELNAME,relNameAttr,EQ);
        ...
        if(!strcmp(attrCatEntryRecord[ATTRCAT_ATTR_NAME_INDEX].sVal,oldName)){
            attrToRenameRecId=RecId{attrToRenameRecId.block,attrToRenameRecId.slot};
            break;
        }
    ...
    RecBuffer AttrBuff(attrToRenameRecId.block);
    Attribute attr[ATTRCAT_NO_ATTRS];
    AttrBuff.getRecord(attr,attrToRenameRecId.slot);
    strcpy(attr[ATTRCAT_ATTR_NAME_INDEX].sVal,newName);
    //   update the AttrName of the record with newName
    //   set back the record with RecBuffer.setRecord
    AttrBuff.setRecord(attr,attrToRenameRecId.slot);
...
int BlockAccess::insert(int relId, Attribute *record) {
    ...
    int blockNum = relcat.firstBlk  /* first record block of the relation (from the rel-cat entry)*/;

    // rec_id will be used to store where the new record will be inserted
    RecId rec_id = {-1, -1};
  
    int numOfSlots = relcat.numSlotsPerBlk/* number of slots per record block */;
    int numOfAttributes =relcat.numAttrs /* number of attributes of the relation */;
   ...
   unsigned char * slotMap=(unsigned char *)malloc(sizeof(unsigned char)* numOfSlots);
    while (blockNum != -1) {
        ...
        RecBuffer Buff(blockNum);
        // get header of block(blockNum) using RecBuffer::getHeader() function
        
        Buff.getHeader(&head1);
        ...
        for(int slot=0;slot<numOfSlots;slot++){
            if(slotMap[slot]==SLOT_UNOCCUPIED){
                rec_id.block=blockNum;rec_id.slot=slot;
                break;
                break;
            }
        }
    ...
    if(rec_id.block==-1 && rec_id.slot==-1)
    {
       ...
       rbuffer.setSlotMap(slotMap);
        ...
        if(prevBlockNum!=-1)
        {
            ...
            HeadInfo head2;
            prevBuffer.getHeader(&head2);
            head2.rblock=rec_id.block;
            // update the rblock field of the header to the new block
            // number i.e. rec_id.block
            prevBuffer.setHeader(&head2);
            // (use BlockBuffer::setHeader() function)
        }
        else            // else
        {
            relcat.firstBlk=rec_id.block;
            // update first block field in the relation catalog entry to the
           
            // new block (using RelCacheTable::setRelCatEntry() function)
        }
           
        relcat.lastBlk=rec_id.block;
    ...
    recbuf.setSlotMap(slotMap);
     HeadInfo bhead;
    recbuf.getHeader(&bhead);
  
    bhead.numEntries++;
    recbuf.setHeader(&bhead);
...
int BlockAccess::search(int relId, Attribute *record, char attrName[ATTR_SIZE], Attribute attrVal, int op) {
    ...
    /* Copy the record with record id (recId) to the record buffer (record)
       ...
       call the appropriate method to fetch the record
    */
    RecBuffer searchblk(recId.block);
    searchblk.getRecord(record,recId.slot);
